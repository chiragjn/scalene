...
class Scalene():
    ...
    def output_profiles():
        ...
        all_instrumented_files = list(set(
            list(Scalene.cpu_samples_python.keys()) + list(Scalene.memory_free_samples.keys()) + list(
                Scalene.memory_malloc_samples.keys())))
        with Scalene.file_or_stdout(Scalene.output_file) as out:
            for fname in sorted(all_instrumented_files):
                ...
                print("%s: %% of CPU time = %6.2f%% out of %6.2fs." % (fname, percent_cpu_time, Scalene.elapsed_time),
                      file=out)
                print("  \t | %9s | %9s | %s %s " % ('CPU %', 'CPU %', 'Avg memory  |' if did_sample_memory else '',
                                                     'Memory      |' if did_sample_memory else ''), file=out)
                print("  Line\t | %9s | %9s | %s%s [%s]" % (
                '(Python)', '(native)', 'growth (MB) |' if did_sample_memory else '',
                ' usage (%)   |' if did_sample_memory else '', fname), file=out)
                print("-" * 80, file=out)
                ...
                with open(fname, 'r') as source_file:
                    for line_no, line in enumerate(source_file, 1):
                        ...
                        # Now, memory stats.
                        n_free_mb = (Scalene.memory_free_samples[fname][line_no] * Scalene.free_sampling_rate) / (
                                    1024 * 1024)
                        ...
                        n_malloc_mb = (Scalene.memory_malloc_samples[fname][
                                           line_no] * Scalene.malloc_sampling_rate) / (1024 * 1024)
                        ...
                        n_usage_mb = 0 if Scalene.total_memory_malloc_samples == 0 else \
                        Scalene.memory_malloc_samples[fname][line_no] / Scalene.total_memory_malloc_samples
    ...
    def main():
        ...
        parser.add_argument('--wallclock', dest='wallclock', action='store_const', const=True, default=False,
...
def env_setup(memory=False, libscalene_path=None):
    ...
    if memory:
        ...
        ld_key = 'DYLD_INSERT_LIBRARIES' if sys.platform.system() == 'Darwin' else 'LD_PRELOAD'
        keys = {
